...
func (rbs RecordBatches) Encode(pe *legacy_encoder.Encoder) {
	for _, rb := range rbs {
		rb.Encode(pe)
	}
}
...
func (rb RecordBatch) Encode(pe *legacy_encoder.Encoder) {
...
func (rb *RecordBatch) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
	...
	if computedChecksum != uint32(rb.CRC) {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("CRC mismatch: calculated %08x, expected %08x", computedChecksum, uint32(rb.CRC)), "crc")
	}
	...
	if numRecords < 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Count of Records cannot be negative: %d", numRecords))
	}
...
func (r Record) Encode(pe *legacy_encoder.Encoder) {
...
func (r *Record) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
...
func (rh RecordHeader) Encode(pe *legacy_encoder.Encoder) {
...
func (rh *RecordHeader) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
	keyLength, err := pd.GetSignedVarint()
	if err != nil {
		if decodingErr, ok := err.(*legacy_errors.PacketDecodingError); ok {
			return decodingErr.WithAddedContext("key_length")
