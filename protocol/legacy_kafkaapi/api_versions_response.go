...
func (a *ApiKeyEntry) Decode(pd *legacy_decoder.Decoder, version int16, logger *logger.Logger, indentation int) (err error) {
	a.Version = version
...
func (r *ApiVersionsResponseBody) Decode(pd *legacy_decoder.Decoder, version int16, logger *logger.Logger, indentation int) (err error) {
	r.Version = version
	...
	if numApiKeys < 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Count of ApiKeys cannot be negative: %d", numApiKeys))
	}
	...
	if pd.Remaining() != 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("unexpected %d bytes remaining in decoder after decoding ApiVersionsResponseBody", pd.Remaining()))
	}
