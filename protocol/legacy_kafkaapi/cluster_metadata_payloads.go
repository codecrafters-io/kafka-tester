...
func (p *ClusterMetadataPayload) Decode(data []byte) (err error) {
	partialDecoder := legacy_decoder.Decoder{}
	...
	case 23:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "BEGIN_TRANSACTION_RECORD")
		}
	case 21:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "ZK_MIGRATION_STATE_RECORD")
		}
	case 2:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "TOPIC_RECORD")
		}
	case 12:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "FEATURE_LEVEL_RECORD")
		}
	case 24:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "END_TRANSACTION_RECORD")
		}
	...
	case 3:
		...
		if partialDecoder.Remaining() > 0 {
			return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Remaining bytes after decoding: %d", partialDecoder.Remaining()), "PARTITION_RECORD")
		}
	}
...
func (p ClusterMetadataPayload) Encode(pe *legacy_encoder.Encoder) {
	...
	case *TopicRecord:
		...
		uuidBytes, err := legacy_encoder.EncodeUUID(record.TopicUUID)
		if err != nil {
			panic(err)
		}
	...
	case *PartitionRecord:
		...
		if p.Version >= 1 {
			...
			for i := range record.Directories {
				uuidBytes, err := legacy_encoder.EncodeUUID(record.Directories[i])
				if err != nil {
					panic(err)
				}
				pe.PutRawBytes(uuidBytes)
...
//lint:ignore U1000, these are not used in the codebase currently
func getUUID(pd *legacy_decoder.Decoder) (string, error) {
	topicUUIDBytes, err := pd.GetRawBytes(16)
	if err != nil {
		return "", err
	}
	topicUUID, err := legacy_encoder.DecodeUUID(topicUUIDBytes)
	if err != nil {
		return "", err
	}
