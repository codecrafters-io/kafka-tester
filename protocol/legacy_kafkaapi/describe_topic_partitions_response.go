...
func (a *DescribeTopicPartitionsResponse) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
...
func (a *DescribeTopicPartitionsResponseTopic) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
...
func (d *DescribeTopicPartitionsResponsePartition) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
...
func (c *DescribeTopicPartitionsResponseCursor) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) error {
	var err error

	// This field is nullable, the first byte indicates whether it's null or not
	checkPresence, err := pd.GetInt8()
	if err != nil {
		if _, ok := err.(*legacy_errors.PacketDecodingError); ok {
			customError := legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected either 0xFF (cursor is null) or 0x01 (cursor is not null), got 0x%02X", uint8(checkPresence)), "RAW_BYTES")
