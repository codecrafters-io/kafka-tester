...
func (r *FetchResponse) Decode(pd *legacy_decoder.Decoder, version int16, logger *logger.Logger, indentation int) (err error) {
	...
	if numResponses < 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Count of TopicResponses cannot be negative: %d", numResponses))
	}
	...
	if pd.Remaining() != 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("unexpected %d bytes remaining in decoder after decoding FetchResponse", pd.Remaining()))
	}
...
func (tr *TopicResponse) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
	...
	if numPartitions < 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Count of PartitionResponses cannot be negative: %d", numPartitions))
	}
...
func (pr *PartitionResponse) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
	...
	if numAbortedTransactions < 0 {
		return legacy_errors.NewPacketDecodingError(fmt.Sprintf("Count of AbortedTransactions cannot be negative: %d", numAbortedTransactions))
	}
...
func (ab *AbortedTransaction) Decode(pd *legacy_decoder.Decoder, logger *logger.Logger, indentation int) (err error) {
