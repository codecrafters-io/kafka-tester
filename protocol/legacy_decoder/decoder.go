...
func (d *Decoder) GetInt8() (int8, error) {
	if d.Remaining() < 1 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int8 length to be 1 byte, got %d bytes", rem), "INT8")
	}
...
func (d *Decoder) GetInt16() (int16, error) {
	if d.Remaining() < 2 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int16 length to be 2 bytes, got %d bytes", rem), "INT16")
	}
...
func (d *Decoder) GetInt32() (int32, error) {
	if d.Remaining() < 4 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int32 length to be 4 bytes, got %d bytes", rem), "INT32")
	}
...
func (d *Decoder) GetInt64() (int64, error) {
	if d.Remaining() < 8 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int64 length to be 8 bytes, got %d bytes", rem), "INT64")
	}
...
func (d *Decoder) GetFloat64() (float64, error) {
	if d.Remaining() < 8 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected float64 length to be 8 bytes, got %d bytes", rem), "FLOAT64")
	}
...
func (d *Decoder) GetUnsignedVarint() (uint64, error) {
	...
	if n == 0 {
		return 0, legacy_errors.NewPacketDecodingError("Unexpected end of data", "UNSIGNED_VARINT")
	}

	if n < 0 {
		return 0, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Unexpected unsigned varint overflow after decoding %d bytes", -n), "UNSIGNED_VARINT")
	}
...
func (d *Decoder) GetSignedVarint() (int64, error) {
	...
	if n == 0 {
		return -1, legacy_errors.NewPacketDecodingError("Unexpected end of data", "SIGNED_VARINT")
	}
	if n < 0 {
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Unexpected varint overflow after decoding %d bytes", -n), "SIGNED_VARINT")
	}
	d.offset += n
	return decodedInteger, nil
}
...
func (d *Decoder) GetArrayLength() (int, error) {
	if d.Remaining() < 4 {
		rem := d.Remaining()
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected array length prefix to be 4 bytes, got %d bytes", rem), "ARRAY_LENGTH")
	}
	...
	if arrayLength > d.Remaining() {
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expect to read at least %d bytes, but only %d bytes are remaining", arrayLength, d.offset), "array length")
	} else if arrayLength > 2*math.MaxUint16 {
		return -1, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Invalid array length: %d", arrayLength), "ARRAY_LENGTH")
	}
...
func (d *Decoder) GetBool() (bool, error) {
	...
	if decodedBool != 1 {
		return false, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected bool to be 1 or 0, got %d", decodedBool), "BOOLEAN")
	}
	return true, nil
}
...
func (d *Decoder) GetStringLength() (int, error) {
	...
	switch {
	case n < -1:
		return 0, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected string length to be >= -1, got %d", n), "STRING_LENGTH")
	case n > d.Remaining():
		return 0, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expect to read at least %d bytes, but only %d bytes are remaining", n, d.offset), "STRING_LENGTH")
	}
...
func (d *Decoder) GetString() (string, error) {
	...
	if d.Remaining() < length {
		rem := d.Remaining()
		return "", legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected string length to be %d bytes, got %d bytes", length, rem), "STRING")
	}
...
func (d *Decoder) GetNullableString() (*string, error) {
	...
	if d.Remaining() < length {
		rem := d.Remaining()
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected string length to be %d bytes, got %d bytes", length, rem), "NULLABLE_STRING")
	}
...
func (d *Decoder) GetCompactString() (string, error) {
	...
	if length < 0 {
		return "", legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected COMPACT_STRING length to be > 0, got %d", length), "COMPACT_STRING")
	}

	if d.Remaining() < length {
		rem := d.Remaining()
		return "", legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected string length to be %d bytes, got %d bytes", length, rem), "COMPACT_STRING")
	}
...
func (d *Decoder) GetCompactNullableString() (*string, error) {
	...
	if length < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected compact nullable string length to be > 0, got %d", length), "COMPACT_NULLABLE_STRING")
	}

	if d.Remaining() < length {
		rem := d.Remaining()
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected string length to be %d bytes, got %d bytes", length, rem), "COMPACT_NULLABLE_STRING")
	}
...
func (d *Decoder) GetInt32Array() ([]int32, error) {
	...
	if d.Remaining() < 4*arrayLength {
		rem := d.Remaining()
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int32 array length to be %d bytes, got %d bytes", 4*arrayLength, rem), "INT32_ARRAY")
	}
	...
	if arrayLength < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Array length can only be -1 or > 0, got: %d", arrayLength), "INT32_ARRAY")
	}
...
func (d *Decoder) GetInt64Array() ([]int64, error) {
	...
	if d.Remaining() < 8*arrayLength {
		rem := d.Remaining()
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected int64 array length to be %d bytes, got %d bytes", 8*arrayLength, rem), "INT64_ARRAY")
	}
	...
	if arrayLength < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Array length can only be -1 or > 0, got: %d", arrayLength), "INT64_ARRAY")
	}
...
func (d *Decoder) GetStringArray() ([]string, error) {
	...
	if arrayLength < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Array length can only be -1 or > 0, got: %d", arrayLength), "STRING_ARRAY")
	}
...
func (d *Decoder) GetRawBytes(length int) ([]byte, error) {
	if length < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected length to be >= 0, got %d", length), "RAW_BYTES")
	} else if length > d.Remaining() {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected length to be lesser than remaining bytes (%d), got %d", d.Remaining(), length), "RAW_BYTES")
	}
...
func (d *Decoder) GetRawBytesFromOffset(length int) ([]byte, error) {
	if length < 0 {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected length to be >= 0, got %d", length), "RAW_BYTES_FROM_OFFSET")
	}

	if d.offset >= len(d.bytes) {
		return nil, legacy_errors.NewPacketDecodingError(fmt.Sprintf("Expected offset to be less than length of raw bytes (%d), got %d", len(d.bytes), d.offset), "RAW_BYTES_FROM_OFFSET")
