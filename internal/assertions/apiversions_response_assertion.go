...
type ApiVersionsResponseAssertion__Updated struct {
	ActualValue   kafkaapi.ApiVersionsResponse__Updated
	ExpectedValue kafkaapi.ApiVersionsResponse__Updated
}

func NewApiVersionsResponseAssertion__Updated(actualValue kafkaapi.ApiVersionsResponse__Updated, expectedValue kafkaapi.ApiVersionsResponse__Updated) *ApiVersionsResponseAssertion__Updated {
	return &ApiVersionsResponseAssertion__Updated{
		ActualValue:   actualValue,
		ExpectedValue: expectedValue,
	}
}

func (a *ApiVersionsResponseAssertion__Updated) assertBody(logger *logger.Logger) error {
	expectedErrorCodeName, ok := errorCodes[int(a.ExpectedValue.Body.ErrorCode)]
	if !ok {
		panic(fmt.Sprintf("CodeCrafters Internal Error: Expected %d to be in errorCodes map", a.ExpectedValue.Body.ErrorCode))
	}
...
func (a *ApiVersionsResponseAssertion__Updated) assertAPIKeysArray(logger *logger.Logger) error {
	if len(a.ActualValue.Body.ApiKeys) < len(a.ExpectedValue.Body.ApiKeys) {
		return fmt.Errorf("Expected API keys array to include atleast %d keys, got %d", len(a.ExpectedValue.Body.ApiKeys), len(a.ActualValue.Body.ApiKeys))
	}
	logger.Successf("✓ API keys array length: %d", len(a.ActualValue.Body.ApiKeys))
...
func (a *ApiVersionsResponseAssertion__Updated) Run(logger *logger.Logger) error {
	if err := NewResponseHeaderAssertion__Updated(a.ActualValue.Header, a.ExpectedValue.Header).Run(logger); err != nil {
		return err
	}
