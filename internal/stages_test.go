...
func normalizeTesterOutput(testerOutput []byte) []byte {
	replacements := map[string][]*regexp.Regexp{
		...
		"index":                      {regexp.MustCompile(`- .index \([0-9]{1,}\)`)},
		...
		"responses Name":             {regexp.MustCompile(`✓ responses\[[0-9]{1,}\].name: [A-Za-z -]{3,}`)},
		"partition_responses length": {regexp.MustCompile(`✓ partition_responses.length: /([0-9]{1,}/)`)},
		"log file":                   {regexp.MustCompile(`/tmp/kraft-combined-logs/[a-zA-Z]{3}-[0-9]{1,}/00000000000000000000.log`)},
