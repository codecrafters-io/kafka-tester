...
func testProduceForMultiplePartitions(stageHarness *test_case_harness.TestCaseHarness) error {
	...
	numPartitions := random.RandomInt(2, 4)

	// Create partition generation config
	partitionConfigs := generateEmptyPartitionConfigs(numPartitions)
		...
		TopicGenerationConfigList: []kafka_files_generator.TopicGenerationConfig{
			{
				...
				PartitonGenerationConfigList: partitionConfigs,
			},
		},
	})
