...
func SendFetchResponse(request *FetchRequest, batches []RecordBatch) []byte {
	// Tamper with CRC32 of each record batch: For the tests
	tamperedBatches := TamperCRC32(batches)

	response := CreateFetchResponse(request, tamperedBatches)

	return response.EncodeResponse()
}

// TamperCRC32 corrupts the CRC32 field of each record batch
func TamperCRC32(batches []RecordBatch) []RecordBatch {
	tamperedBatches := make([]RecordBatch, len(batches))

	for i, batch := range batches {
		// Copy the batch
		tamperedBatch := batch

		// Corrupt the CRC32 by flipping all bits (XOR with 0xFFFFFFFF)
		originalCRC := uint32(batch.CRC)
		tamperedCRC := originalCRC ^ 0xFFFFFFFF
		tamperedBatch.CRC = int32(tamperedCRC)

		tamperedBatches[i] = tamperedBatch
	}

	return tamperedBatches
}
